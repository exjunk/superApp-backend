...
def manage_file_download(url, file_path):
    ...
    if not is_today(file_path):
        ...
        if os.path.exists(file_path):
            ...
            print(f"Renamed {file_path} to {temp_file_path}")

        # Attempt to download the new file
        if download_file(url, file_path):
            print(f"Downloaded new file to {file_path}")
            ...
            if os.path.exists(temp_file_path):
                ...
                print(f"Restored original file from {temp_file_path} to {file_path}")
    else:
        print(f"File {file_path} was already downloaded today. No action taken.")
