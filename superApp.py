...
class Index_attributes:
        def get_index_attributes(name)->(Index_config):
             if(name == Index.BANKNIFTY.name): 
                 return Index_config(name,100,15,'NSE_FNO',socket.bankNifty_current_price)
             if(name == Index.SENSEX.name): 
                return Index_config(name,100,10,'BSE_FNO',socket.sensex_current_price)
             if(name == Index.NIFTY.name): 
                return Index_config(name,50,25,'NSE_FNO',socket.nifty_current_price)
             if(name == Index.FINNIFTY.name): 
                return Index_config(name,50,40,'NSE_FNO',socket.finnifty_current_price)
